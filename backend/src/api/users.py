...
from src.models.user import User, UserAuth, UserLoginResponse, UserNoPass, UserRegister
...
async def login(user_auth: UserAuth):
    ...
    return UserLoginResponse(username=user.username, scope=user.scope, token=token)
